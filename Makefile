...
SBI_DIR_SRCS = $(shell find $(SBI_DIR) -name "*.c")
...
SBI_DIR_SRCS_OBJS = $(addprefix $(OBJECT_DIR)/, $(SBI_DIR_SRCS:.c=.o))
...
$(TARGET): $(KERNEL_DIR_ASSEMS_OBJS) $(RISCV32_DIR_ASSEMS_OBJS) $(KERNEL_DIR_SRCS_OBJS) $(RISCV32_DIR_SRCS_OBJS) $(RIBS_DIR_SRCS_OBJS) $(SBI_DIR_SRCS_OBJS)
	$(CC) $(CFLAGS) -Wl,-T$(LINKER_DIR)/kernel.ld -Wl,-Map=$(LINKER_DIR)/kernel.map -o $(TARGET) $(KERNEL_DIR_ASSEMS_OBJS) $(RISCV32_DIR_ASSEMS_OBJS) $(KERNEL_DIR_SRCS_OBJS) $(RISCV32_DIR_SRCS_OBJS) $(RIBS_DIR_SRCS_OBJS) $(SBI_DIR_SRCS_OBJS)
...
$(SBI_DIR_SRCS_OBJS): $(SBI_DIR_SRCS)
	$(CC) $(CFLAGS) -c $(<D)/$(patsubst %.o,%.c,$(@F)) -o $@
